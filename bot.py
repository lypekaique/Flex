...
async def configurar(interaction: discord.Interaction, tipo: str = None, canal: discord.TextChannel = None, cargo: discord.Role = None):
    ...
    if tipo not in ['alertas', 'score', 'comandos', 'live', 'votacao', 'top_flex', 'piorzin']:
        await interaction.followup.send(
...
async def send_champion_ban_notification(account_id: int, champion_name: str, ban_days: int, ban_level: int, ban_reason: str, pintado_count: int = 0):
    ...
    try:
        ...
        for guild in bot.guilds:
            ...
            # Busca canal de ALERTAS (não canal de score)
...
async def send_mvp_voting(game_id: str, guild: discord.Guild, players: List[Dict]):
    ...
    try:
        ...
        is_victory = bool(first_player and first_player.get('win', False))
...
async def check_live_games():
    ...
    try:
        ...
        if games_map:
            print(f"\n📋 [Live Games] FASE 1 concluída - {len(games_map)} partida(s) detectada(s):")
            ...
            try:
                ...
                if existing_message and existing_message.get('message_id'):
                    ...
                    try:
                        ...
                        gld_id = existing_message.get('guild_id')
                        
                        if msg_id and ch_id and gld_id:
                            guild = bot.get_guild(int(gld_id))
                            if guild:
